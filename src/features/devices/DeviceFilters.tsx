    ...
    <section aria-label="Device filters" className="border border-app-hairline bg-app-surface p-4 shadow-sm">
      <div className="flex flex-wrap items-end gap-3">
        <div className="min-w-[240px] flex-1">
          ...
          <input
            ...
            className="w-full rounded-md border border-app-hairline px-3 py-2 text-sm outline-none focus:border-[#0078d4]"
          />
        </div>
        ...
        <label className="inline-flex items-center gap-2 rounded-md border border-[#0078d4]/40 bg-[#0078d4]/5 px-3 py-2 text-sm font-medium">
          <input
            type="checkbox"
        ...
        <button type="button" onClick={resetFilters} className="px-2 py-2 text-sm font-medium text-[#005a9e] hover:underline">Reset filters</button>
      </div>
      <div className="mt-3 flex min-h-6 flex-wrap gap-2">
        {[...filters.operatingSystem.map((value) => `OS: ${value}`), ...filters.managementState.map((value) => `State: ${value}`), ...filters.managedDeviceOwnerType.map((value) => `Owner: ${value}`), ...(filters.complianceOnly ? ['Compliant only'] : []), ...(filters.duplicatesOnly ? ['Duplicates only'] : [])].map((chip) => (
          <span key={chip} className="border border-[#0078d4]/30 bg-[#0078d4]/5 px-2 py-1 text-xs font-medium text-[#005a9e]">{chip}</span>
        ))}
      </div>
    </section>
  );
