...
function IntuneMark() {
  return (
    <div className="flex items-center gap-3" aria-label="Microsoft Intune">
      <img src="/intune-logo.svg" alt="Microsoft Intune" className="h-8 w-8" />
      <span className="text-base font-semibold tracking-tight">Intune</span>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-app-background">
      <header className="border-b border-app-hairline bg-app-surface">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-6 py-4">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h1 className="text-base font-semibold">Duplicate Device Cleanup</h1>
              {account?.username && <p className="text-xs text-app-ink/60">{account.username}</p>}
            </div>
          </div>
      ...
      <div className="mx-auto max-w-7xl px-6 py-7">
        <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-app-keeper">Device management</p>
        ...
        <nav className="mb-6 flex flex-wrap gap-1 border-b border-app-hairline" aria-label="Screens">
          {SCREENS.map((s) => (
            <button key={s.id} type="button" onClick={() => setScreen(s.id)} aria-current={screen === s.id ? 'page' : undefined}
              className={`border-b-2 px-4 py-2.5 text-sm font-medium transition-colors ${screen === s.id ? 'border-[#0078d4] text-[#005a9e]' : 'border-transparent text-app-ink/65 hover:border-app-hairline hover:text-app-ink'}`}>
              {s.label}
            </button>
          ))}
        </nav>
